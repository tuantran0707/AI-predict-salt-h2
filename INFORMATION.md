...
in-house dataset (**84 corroded samples + 181 clean samples**, 265 total).
...
Defined in `detect_salt.py`, function `salt_color_ratio()`. Four masks
...
| **Yellow/amber crust** | 8 < H < 55, S > 25, V > 70 | Warm-light yellow sulfate crust |
| **Brown crust** | 5 < H < 35, S > 20, 45 < V < 160 | Oxidized dark sulfate deposits |
...
confidence = 0.72 * ai_score + 0.28 * cv_score
```
...
(2) cv_ratio >= 0.03  AND  margin >= -0.02   # fallback full-frame mode
(3) Pole mode (default): per-terminal AI+HSV rules with
    pole_cv_threshold=0.10 and yellow-aware gates
```
...
| `pole_cv_threshold` | 0.10 | Too many pole false positives | Weak pole corrosion is missed |
| `fusion_weight_ai` | 0.72 | Dataset is well-representative | CV mask is more reliable than AI |
...
detector = SaltDetector(
    ...
    margin_threshold=0.05,    # stricter AI gate
    cv_threshold=0.05,        # stricter full-frame HSV gate
    pole_cv_threshold=0.12,   # stricter terminal HSV gate
    fusion_weight_ai=0.75,
)
```
